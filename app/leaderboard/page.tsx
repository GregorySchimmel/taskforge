...
export default function LeaderboardPage() {
  const { leaderboard, state, canDiscoverTalent } = useApp();
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Top talent ranked by verified points — devs, designers & editors</p>
        </div>
      </div>
      ...
      {!canDiscoverTalent ? (
        <TalentPaywall>
          <div className="space-y-3">
            {filtered.slice(0, 3).map((dev) => (
              <Card key={dev.id} className="border-border/50">
                <CardContent className="flex items-center gap-4 p-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{dev.stats.totalPoints} points</p>
                  </div>
                </CardContent>
              </Card>
            ))}
      ...
      <Dialog open={!!hireTarget} onOpenChange={() => setHireTarget(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Simulated contact flow. In production, integrate in-app messaging or email.
          </p>
          <div className="rounded-lg border border-border bg-secondary/30 p-4 text-sm">
            <p><strong>Email:</strong> {hireTarget?.email}</p>
            <p><strong>Portfolio:</strong> {hireTarget?.portfolioUrl}</p>
          </div>
